@extends('inventory.layouts.app', ['page' => __('inventory.product_selector'), 'pageSlug' => 'admincarts', 'section' => 'documents', 'search' => 'admincarts'])
                ...
                <div class="row">
                    ...
                    <div class="col-5 text-right">
						<form method="post" action="{{ route('client_orders.store') }}">
							...
							<button type="submit" title="Client order" class="btn btn-simple btn-sm btn-sale @if(!$admincart->finalized_at) disabled @endif"><i class="fas fa-file-download"></i></button>
						</form>
                    </div>
                ...
                response.forEach(item => {
                    tbody +=
                        `<tr><td style="width: 15%;">${item.article}</a></td>
                            ...
                            <td style="width: 40%;"><a href="/products/${item.id}" title = "${item.description}" data-toggle="tooltip" data-placement="bottom" target="_blank">${item.name}</a></td>
                            <td style="width: 15%;">${item.stock}</td>
                            <td style="width: 15%;">${item.price}</td>
                            <td><button type="button" class="btn btn-simple btn-sm btn-selector" id="${item.id}" OnClick="admincart_add_product('`+{{$admincart->id}}+`','${item.id}')"><i class="fas fa-check-double"></i></button></td>
                            </tr>`;
                })
...
document.addEventListener("DOMContentLoaded", () => 
{
    ...
    var KTTreeview = function ()
    {
        var initiate_tree = function()
        {
            ...
            {
                $.ajax({
                    url: '{{route('admincarts.products.filter.by.group')}}',
                    type: 'POST',
                    ...
                    success:function(data)
                    {
                        ...
                        for(var i = 0; i < data.length; i++)
                        {
                            ...
                            html += `<td><a href="/products/`+data[i].id+`" target="_blank" data-toggle="tooltip" data-placement="bottom" title="@lang("inventory.edit")">`+data[i].name+`</a></td>`;
                            html += `<td>`+data[i].stock+`</td>`;
                            html += `<td>`+data[i].price+`</td>`;
                            html += `<td><button type="button" class="btn btn-simple btn-sm btn-selector" id="`+data[i].id+`" OnClick="admincart_add_product('`+{{$admincart->id}}+`','`+data[i].id+`')"><i class="fas fa-check-double"></i></button></td>`;
                            html += `</tr>`;
                        }
