                    ...
                    <div class="pl-lg-4">
                        <div class="form-group{{ $errors->has('type') ? ' has-danger' : '' }}">
                            ...
                            <select name="type" id="input-method" class="form-select form-control-alternative{{ $errors->has('type') ? ' is-invalid' : '' }}" required>
                                @if(isset($receipt->id))
                                <option value="income" selected>{{__('inventory.payment_for_sale')}}</option>
                                @else
                                @foreach (['income' => __('inventory.payment_for_sale'), 'expense' => 'Возврат оплаты покупателю'] as $type => $title)
                                    @if($type == old('type'))
                        ...
                        <div class="form-group{{ $errors->has('reference') ? ' has-danger' : '' }}">
                            ...
                            @if($sale->id)
                                <input type="text" name="reference" id="input-reference" class="form-control form-control-alternative{{ $errors->has('reference') ? ' is-invalid' : '' }}" placeholder="{{__('inventory.reference_doc')}}" value="{{__('inventory.sale')}} № {{ $sale->id }}">
                            @else
