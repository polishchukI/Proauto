@extends('inventory.layouts.app', ['page' => __('inventory.new_transaction'), 'pageSlug' => 'client_order_corrections', 'section' => 'inventory', 'search' => 'client_order_corrections'])
@section('content')
<div class="row">
            ...
            <div class="card-header">
                <div class="row align-items-center">
                    ...
                    <div class="col-4 text-right">
                        <a href="{{ route('client_order_corrections.show', $client_order_correction) }}" class="btn btn-sm btn-simple btn-back"><i class="fas fa-arrow-left"></i></a>
                    </div>
                </div>
            </div>
            <div class="card-body">
                <form method="post" action="{{ route('client_order_corrections.transaction.store', $client_order_correction) }}" autocomplete="off">
                    @csrf
                    <input type="hidden" name="client_order_correction_id" value="{{ $client_order_correction->id }}">
                    <input type="hidden" name="client_id" value="{{ $client_order_correction->client_id }}">
                    ...
                    <div class="pl-lg-4">
                        <div class="form-group{{ $errors->has('type') ? ' has-danger' : '' }}">
                            ...
                            <select name="type" id="input-method" class="form-select form-control-alternative{{ $errors->has('type') ? ' is-invalid' : '' }}" required>
                                @if(isset($receipt->id))
                                <option value="income" selected>{{__('inventory.payment_for_client_order_correction')}}</option>
                                @else
                                @foreach (['income' => __('inventory.payment_for_client_order_correction'), 'expense' => 'Возврат оплаты покупателю'] as $type => $title)
                                    @if($type == old('type'))
                        ...
                        <div class="form-group{{ $errors->has('reference') ? ' has-danger' : '' }}">
                            ...
                            @if($client_order_correction->id)
                                <input type="text" name="reference" id="input-reference" class="form-control form-control-alternative{{ $errors->has('reference') ? ' is-invalid' : '' }}" placeholder="{{__('inventory.reference_doc')}}" value="{{__('inventory.client_order_correction')}} № {{ $client_order_correction->id }}">
                            @else
                        ...
                        <div class="row">
                            <div class="col-9">
                                <div class="form-group{{ $errors->has('amount') ? ' has-danger' : '' }}">
                                    ...
                                    @if($client_order_correction->id)
                                    <input type="number" step=".01" name="amount" id="input-amount" class="form-control form-control-alternative" placeholder="{{__('inventory.reference_doc')}}" value="{{ $client_order_correction->total_amount }}" required>
                                    @else
