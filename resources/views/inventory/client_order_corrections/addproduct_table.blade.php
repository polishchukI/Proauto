@extends('inventory.layouts.app', ['page' => __('inventory.product_selector'), 'pageSlug' => 'client_order_corrections', 'section' => 'documents', 'search' => 'client_order_corrections'])
                ...
                response.forEach(item => {
                    tbody +=
                        `<tr><td style="width: 15%;">${item.article}</a></td>
                            ...
                            <td><button type="button" class="btn btn-simple btn-sm btn-selector" id="${item.id}" OnClick="client_order_correction_add_product('`+{{$client_order_correction->id}}+`','${item.id}')"><i class="fas fa-check-double"></i></button></td>
                            </tr>`;
                })
...
document.addEventListener("DOMContentLoaded", () => 
{
    ...
    var KTTreeview = function ()
    {
        var initiate_tree = function()
        {
            ...
            {
                $.ajax({
                    url: '{{route('client_order_corrections.products.filter.by.group')}}',
                    type: 'POST',
                    ...
                    success:function(data)
                    {
                        ...
                        for(var i = 0; i < data.length; i++)
                        {
                            ...
                            html += `<td><button type="button" class="btn btn-simple btn-sm btn-selector" id="`+data[i].id+`" OnClick="client_order_correction_add_product('`+{{$client_order_correction->id}}+`','`+data[i].id+`')"><i class="fas fa-check-double"></i></button></td>`;
                            html += `</tr>`;
                        }
