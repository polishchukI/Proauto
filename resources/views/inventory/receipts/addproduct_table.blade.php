@extends('inventory.layouts.app', ['page' => __('inventory.product_selector'), 'pageSlug' => 'receipts', 'section' => 'documents', 'search' => 'receipts'])
            ...
            <div class="card-header">
                <div class="row">
                    ...
                    <div class="col-6 text-right">
                        ...
                        <a href="{{ route('receipts.show', ['receipt' => $receipt]) }}" class="btn btn-simple btn-sm btn-back"><i class="fas fa-arrow-left"></i></a>
                    </div>
                </div>
            </div>
            <div class="card-body" style="max-height:100%;overflow:auto;">
                ...
                response.forEach(item => {
                    tbody +=
                        `<tr><td style="width: 15%;">${item.article}</a></td>
                            ...
                            <td><button type="button" class="btn btn-simple btn-sm btn-selector" id="${item.id}" OnClick="receipt_add_product('`+{{$receipt->id}}+`','${item.id}')"><i class="fas fa-check-double"></i></button></td>
                            </tr>`;
                })
    ...
    var KTTreeview = function ()
    {
        var initiate_tree = function()
        {
            ...
            .bind("select_node.jstree", function(evt, data)
            {
                ...
                $.ajax({
                    url: '{{route('receipts.products.filter.by.group')}}',
                    type: 'POST',
                    ...
                    success:function(data)
                    {
                        ...
                        for(var i = 0; i < data.length; i++)
                        {
                            ...
                            html += `<td>`;
                            html +=`<a href="/products/`+data[i].id+`" target="_blank" class="btn btn-link text-warning" data-toggle="tooltip" data-placement="bottom" title="Edit Product">`;
                            html += data[i].name;
                            html += `</a></td>`;
                            html += `<td>`+data[i].stock+`</td>`;
                            html += `<td>`+data[i].price+`</td>`;
                            html += `<td><button type="button" class="btn btn-simple btn-sm btn-selector" id="`+data[i].id+`" OnClick="receipt_add_product('`+{{$receipt->id}}+`','`+data[i].id+`')"><i class="fas fa-check-double"></i></button></td>`;
                            html += `</tr>`;
                        }
