@extends('inventory.layouts.app', ['page' => 'Manage Sale', 'pageSlug' => 'sales', 'section' => 'documents', 'search' => 'sales'])
            ...
            <div class="card-header">
                <div class="row">
                    ...
                    <div class="col-4 text-right">
                        <a href="{{ route('sales.show', ['sale' => $sale]) }}" class="btn btn-simple btn-sm text-info"><i class="fas fa-arrow-left"></i>{{ __('inventory.back_to_document') }}</a>
                    </div>
                </div>
            </div>
            <div class="card-body" style="max-height:100%;overflow:auto;">
                ...
                <div class="table-full-width table-responsive" id="selectedProductsTable">
                    <table class="table">
                        ...
                        @foreach ($sale->products as $item)
                            <tr id="return_from_the_client_selected_product-{{ $item->product_id }}" class="pointer" ondblclick="return_from_the_client_edit_product('{{$sale->id}}','{{ $item->product_id }}');">
                ...
                $.ajax({
                    ...
                    success:function(data)
                    {
                        ...
                        for(var i = 0; i < data.length; i++)
                        {
                            ...
                            html += `<td><button type="button" class="btn btn-link btn-sm text-success" id="`+data[i].id+`" OnClick="sale_add_product('`+{{$sale->id}}+`','`+data[i].id+`')"><i class="fas fa-check-double"></i></button></td>`;
                            html += `</tr>`;
                        }
