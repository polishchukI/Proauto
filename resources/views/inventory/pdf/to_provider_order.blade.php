...
<html dir="ltr" lang="ru">
	<head>
		...
		<title>{{ __('inventory_doc_print.to_provider_order') }} № {{$to_provider_order['id']}}</title>
    </head>
	<style>
		body {
		...
		<div class="container">
			<div>
				...
				<table class="table">
						<tbody>
							<tr>
								<td style="width: 50%;"><b>{{ __('inventory_doc_print.organisation_name') }}</b> {{$to_provider_order["organisation_name"]}}<br /></td>
								<td style="width: 50%;"><b>{{ __('inventory_doc_print.provider') }}</b> {{$provider["name"]}}<br /></td>
							</tr>
							<tr>
								<td style="width: 50%;"><b>{{ __('inventory_doc_print.organisation_phone') }}</b> {{$to_provider_order["organisation_phone"]}}<br /></td>
								<td style="width: 50%;"><b>{{ __('inventory_doc_print.date') }}</b> {{date('j F, Y', strtotime($to_provider_order["created_at"]))}}<br /></td>
							</tr>
							<tr>
								<td style="width: 50%;"><b>{{ __('inventory_doc_print.organisation_phone') }}</b> {{$to_provider_order["organisation_phone2"]}}<br /></td>
								<td style="width: 50%;"><b>{{ __('inventory_doc_print.currency') }}</b> {{$to_provider_order["currency"]}}<br /></td>
							</tr>
							<tr>
								<td style="width: 50%;"><b>{{ __('inventory_doc_print.organisation_email') }}</b> {{$to_provider_order["organisation_email"]}}<br /></td>
								<td style="width: 50%;"><b>{{ __('inventory_doc_print.to_provider_order') }}</b> № {{$to_provider_order["id"]}}<br /></td>
							</tr>
						</tbody>
					</table>
				</div>
				...
				<div class="table-responsive">
					<table class="table table-bordered">
						...
						<tbody>
							@if($to_provider_order["total_amount"]!= $to_provider_order["subtotal"])
							<tr style="border-top: 1px solid #5a5c69;border-bottom: 1px solid #5a5c69;border-left: 1px solid #5a5c69;border-right: 1px solid #5a5c69;">
								<td class="text-right" colspan="6"><b>{{ __('inventory_doc_print.subtotal') }}</b></td>
								<td class="text-right">{{$to_provider_order["subtotal"]}}</td>
							</tr>
							@endif
							@if($to_provider_order["discountValue"]>0)
							<tr style="border-top: 1px solid #5a5c69;border-bottom: 1px solid #5a5c69;border-left: 1px solid #5a5c69;border-right: 1px solid #5a5c69;">
								<td class="text-right" colspan="6"><b>{{ __('inventory_doc_print.discountValue') }}</b></td>
								<td class="text-right">{{$to_provider_order["discountValue"] ?? 0}}</td>
							</tr>
							@endif
							@if($to_provider_order["tax"]>0)
							<tr style="border-top: 1px solid #5a5c69;border-bottom: 1px solid #5a5c69;border-left: 1px solid #5a5c69;border-right: 1px solid #5a5c69;">
								<td class="text-right" colspan="6"><b>{{ __('inventory_doc_print.tax') }}</b></td>
								<td class="text-right">{{$to_provider_order["tax"]}}</td>
							</tr>
							@endif
							@if($to_provider_order["deliverycost"]>0)
							<tr style="border-top: 1px solid #5a5c69;border-bottom: 1px solid #5a5c69;border-left: 1px solid #5a5c69;border-right: 1px solid #5a5c69;">
								<td class="text-right" colspan="6"><b>{{ __('inventory_doc_print.deliverycost') }}</b></td>
								<td class="text-right">{{$to_provider_order["deliverycost"]??0}}</td>
							</tr>
							@endif
							<tr style="border-top: 1px solid #5a5c69;border-bottom: 1px solid #5a5c69;border-left: 1px solid #5a5c69;border-right: 1px solid #5a5c69;">
								<td class="text-right" colspan="4"><b>{{ __('inventory_doc_print.total') }}</b></td>
								<td class="text-right">{{$to_provider_order["quantity"]}}</td>
								<td class="text-right"></td>
								<td class="text-right">{{$to_provider_order["total_amount"]}}</td>
							</tr>
						</tbody>
					</table>
				</div>
