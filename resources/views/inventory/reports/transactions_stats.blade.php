@extends('inventory.layouts.app', ['pageSlug' => 'tstats', 'page' => 'Transaction Statistics', 'section' => 'transactions'])
						...
						<tbody>
							@foreach ($transactionsperiods as $period => $data)
								<tr>
									...
									<td>{{ $data->where('type', 'payment')->sum('amount') }}</td>
									<td>{{ $data->where('payment_method_id', optional($methods->where('name', 'Cash')->first())->id)->sum('amount') }}</td>
                            ...
                            <tbody>
                                @foreach($clients as $client)
                                    <tr>
                                        ...
                                        <td>
                                            <a href="{{ route('clients.transactions.add', $client) }}" class="btn btn-sm btn-primary" data-toggle="tooltip" data-placement="bottom" title="Register Transation">
                                                <i class="tim-icons icon-simple-add"></i>
                                            </a>
                                            <a href="{{ route('clients.show', $client) }}" class="btn btn-sm btn-primary" data-toggle="tooltip" data-placement="bottom" title="See client">
            ...
            <div class="card shadow mb-4">
                <div class="card-header">
                    <div class="row">
                        ...
                        <div class="col-4 text-right">
                            <a href="{{ route('payment-methods.index') }}" class="btn btn-sm btn-primary">View Methods</a>
                        </div>
                    </div>
                </div>
                <div class="card-body">
                        ...
                        <table class="table">
                            ...
                            <tbody>
                                @foreach($methods as $method)
                                    <tr>
                                        <td><a href="{{ route('payment-methods.show', $method) }}">{{ $method->name }}</a></td>
                                        <td>{{ $transactionsperiods['Year']->where('payment_method_id', $method->id)->count() }}</td>
                                        <td>{{ $transactionsperiods['Year']->where('payment_method_id', $method->id)->sum('amount') }}</td>
                                        <td>
                                            <a href="{{ route('payment-methods.show', $method) }}" class="btn btn-simple btn-sm" data-toggle="tooltip" data-placement="bottom" title="See Method">
                                                <i class="tim-icons icon-zoom-split"></i>
                                            </a>
                                        </td>
                                    </tr>
						...
						<tbody>
							@foreach ($salesperiods as $period => $data)
								<tr>
									...
									<td>{{ $data->where('finalized_at', '!=', null)->map(function ($sale) {return $sale->products->sum('total_amount');})->sum() }}</td>
