<div class="widget widget-filters widget-filters--offcanvas--mobile" data-collapse="" data-collapse-opened-class="filter--opened">
	<form action="{{ route('search.number.filter') }}" method="POST">
	<input type="hidden" name="current_uri" value="{{url()->current()}}">
		@csrf
		...
		<div class="widget-filters__list">
			...
			{{-- <div class="widget-filters__item">
				<div class="filter filter--opened" data-collapse-item="">
					<button type="button" class="filter__title" data-collapse-trigger="">{{ __('catalog.sort') }}
									...
									<div class="filter filter--opened" data-collapse-item="">
										<div class="form-group">										
											<select class="form-control" name="sortBy" onchange="this.form.submit();">
												<option value="brand" @if(Session::has('sortBy') && Session::get('sortBy')=="brand") selected @endisset>{{ __('catalog.brand') }}</option>
												<option value="price" @if(Session::has('sortBy') && Session::get('sortBy')=="brand") selected @endisset>{{ __('catalog.price') }}</option>
												<option value="day" @if(Session::has('sortBy') && Session::get('sortBy')==3) selected @endisset>{{ __('catalog.day') }}</option>
												<option value="available" @if(Session::has('sortBy') && Session::get('sortBy')==available) selected @endisset>{{ __('catalog.available') }}</option>
											</select>
										</div>
									</div>
								</div>
								...
								<div class="filter-list__list">
									@foreach($arProp["values"] as $value => $count)
										<label class="filter-list__item">
											<span class="input-check filter-list__input" onchange="javascript:this.form.submit();">
												<span class="input-check__body">
													<input class="input-check__input" value="{{$value}}" type="checkbox">
								...
								<div class="filter-list__list">
									@foreach ($ResultArray["all_brands"] as $bkey=>$brand)
										<label class="filter-list__item">
											...
											<span class="filter-list__counter">{{Session::get('currency_symbol')}} {{number_format($ResultArray["ab_min_price_f"][$bkey],2)}}</span>
											@endif
										</label>
									@endforeach
								</div>
...
function SetPropFilter(Prop,Value)
{
	SendPostValue(
		{
			FilterProp:Prop, FilterValue:Value, SetFilter:'Y'
		}
	);
}
