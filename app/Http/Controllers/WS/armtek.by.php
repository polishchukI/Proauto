...
if(extension_loaded('curl')){
	...
	$URL = "VKORG=2000&QUERY_TYPE=".$QTp."&KUNNR_RG=".$aWs['client_id']."&PIN=".$ArtNum; //echo $URL.'<br>';
	if($Brand!=''){$URL .='&brand='.$Brand; $WithCrosses=true; }
	$ch = curl_init('http://ws.armtek.by/api/ws_search/search?format=json');
	curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
	curl_setopt($ch, CURLOPT_POST, true);
	curl_setopt($ch, CURLOPT_POSTFIELDS, $URL);
	curl_setopt($ch, CURLOPT_USERPWD, $aWs['login'].':'.$aWs['password'] );
...
{
	echo 'PHP extension "curl" is not loaded';
}
